...
async function getProfileListings(category: string = ''): Promise<string[]> {
  ...
  try {
    ...
    // Najdi všechny profily - různé možné selektory
    const selectors = [
      'a[href*="/profil/"]',
      'a[href*="/escort/"]',
      'a[href*="/privat/"]',
    ...
    for (const selector of selectors) {
      $(selector).each((_, element) => {
        ...
        if (href && (href.includes('/profil/') || href.includes('/escort/') || href.includes('/privat/'))) {
...
async function main() {
  console.log('🚀 Spouštím scraper pro eroguide.cz\n');

  mkdirSync(OUTPUT_DIR, { recursive: true });

  // Získej profily z různých kategorií
  const categories = ['', 'privat', 'escort', 'masaze'];
  let allProfileUrls: string[] = [];

  for (const category of categories) {
    const urls = await getProfileListings(category);
    allProfileUrls = [...allProfileUrls, ...urls];
    await new Promise(resolve => setTimeout(resolve, 1000)); // Delay mezi kategoriemi
  }
  ...
  // Ulož výsledky
  const outputFile = join(OUTPUT_DIR, 'eroguide-data.json');
